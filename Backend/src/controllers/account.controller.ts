...
export async function changePassword(req: Request, res: Response, next: NextFunction) {
	try {
		...
		res.status(204).send({ success: true, data: { sessionRevoked: true, message: "Password updated" } });
	} catch (error) {
		next(error);
	}
}
...
export async function forgotPassword(req: Request, res: Response, next: NextFunction) {
	try {
		...
		res.status(204).send({ success: true, data: { message: "If that email is registered, a reset link has been sent." } });
	} catch (error) {
		next(error);
	}
}
